...
class UserCodecsRoundTripSpec extends AnyFlatSpec with Matchers {
  ...
  "UserCodecs" should "write and read back every persisted type unchanged" in {
    val (covered, findings) = PersistedRoundTrip.registry[UserCodecs.OmittingNone, UserCodecs.WritingNone](tools.IntegrationMongoTarget.from(_root_.settings.ProcessConfiguration.resolve()).get, UserCodecs.registry, Set.empty)
