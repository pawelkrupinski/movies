...
class UserRepositoryIntegrationSpec extends AnyFlatSpec with Matchers with BeforeAndAfterAll with UserStateWritesContract with UserSessionVersionContract {
  ...
  private lazy val isolated = tools.IsolatedMongoDatabase.open(tools.IntegrationMongoTarget.fromEnv(Env.fromProcess()).get, "user-repository")
  ...
  it should "stream a delete through to the change-time cache, even though the event key names no user" in
    tools.IsolatedMongoDatabase.withDatabase(tools.IntegrationMongoTarget.fromEnv(Env.fromProcess()).get, "userstate-stream-delete") { db =>
  ...
  it should "keep its change stream open past a row it cannot decode" in
    tools.IsolatedMongoDatabase.withDatabase(tools.IntegrationMongoTarget.fromEnv(Env.fromProcess()).get, "userstate-malformed") { db =>
  ...
  it should "report lost track again when a dead cursor reopens" in
    tools.IsolatedMongoDatabase.withDatabase(tools.IntegrationMongoTarget.fromEnv(Env.fromProcess()).get, "userstate-reopen") { db =>
