...
class FilterDescriptionSpec extends AnyFlatSpec with Matchers {
  ...
  it should "phrase room include / exclude / summary in English" in {
    ...
    ).title shouldBe "Showtimes — films in screen Sala 5"
    ...
    FilterDescription.forIndex(London, Map("room" -> allButOne), schedules)
      .title shouldBe "Showtimes — films without screens Sala 7"

    val schedulesBig = Seq(
      film("A", Multikino, Seq("Sala 1", "Sala 2", "Sala 3", "Sala 4", "Sala 5", "Sala 6", "Sala 7", "Sala 8"))
    )
    val four = (1 to 4).map(i => s"Multikino Stary Browar|Sala $i")
    FilterDescription.forIndex(London, Map("room" -> four), schedulesBig).title shouldBe "Showtimes — films 4 screens"
  }

  it should "phrase genre / country / director filters in English" in {
    FilterDescription.forIndex(London, Map("genre" -> Seq("Komedia")), schedules).title shouldBe "Showtimes — films genre Komedia"
    FilterDescription.forIndex(London, Map("genre" -> Seq("Animacja", "Komedia")), schedules).title shouldBe "Showtimes — films without genres Dramat"
    FilterDescription.forIndex(London, Map("country" -> Seq("Japonia")), schedules).title shouldBe "Showtimes — films from Japonia"
  ...
  it should "phrase cinema filters with the English 'at' / 'without' prepositions" in {
    ...
    FilterDescription.forIndex(London, Map("cinema" -> Seq(cinemas.head)), schedules)
      .title should startWith ("Showtimes — films at ")
