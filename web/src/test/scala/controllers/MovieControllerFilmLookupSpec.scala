...
/**
 * The film page is addressed by slug (`/{city}/film/{slug}`); the older
...
class MovieControllerFilmLookupSpec extends AnyFlatSpec with Matchers {
  ...
  "GET /film/{slug}" should "resolve a displayed title that contains a single-digit Arabic numeral" in {
    ...
    val result = ctrl.filmBySlug("poznan", "diabel-ubiera-sie-u-prady-2")
      .apply(FakeRequest(GET, "/poznan/film/diabel-ubiera-sie-u-prady-2"))
  ...
  it should "404 a slug that matches no schedule" in {
    ...
    val result = ctrl.filmBySlug("poznan", "no-such-film")
      .apply(FakeRequest(GET, "/poznan/film/no-such-film"))
    status(result) shouldBe NOT_FOUND
  }

  "the legacy GET /film?title=…" should "301 onto the slug address" in {
    ...
    redirectLocation(result) shouldBe Some("/poznan/film/diabel-ubiera-sie-u-prady-2")
  }
  ...
  "the film page <title>" should "lead with the film, its year and a seans keyword" in {
    ...
    val result = ctrl.filmBySlug("poznan", "diuna").apply(FakeRequest(GET, "/poznan/film/diuna"))
  ...
  "the film page" should "preconnect to the poster CDN and prioritise the LCP poster" in {
    val ctrl = buildController("Diuna", Some(2024))
    val html = contentAsString(ctrl.filmBySlug("poznan", "diuna").apply(FakeRequest(GET, "/poznan/film/diuna")))
