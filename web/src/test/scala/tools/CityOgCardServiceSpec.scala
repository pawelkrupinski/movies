...
class CityOgCardServiceSpec extends AnyFlatSpec with Matchers {
  ...
  "CityOgCardService.card" should "build a 1200×630 page-like PNG from the films' posters" in {
    ...
    val bytes = new CityOgCardService(fetch).card("poznan", "Repertuar kin w Poznaniu", "Kinowo", "kinowo.net",fiveFilms)
  ...
  it should "memoise per city — second call neither re-fetches nor re-renders" in {
    ...
    val a = service.card("poznan", "Repertuar kin w Poznaniu", "Kinowo", "kinowo.net",fiveFilms)
    val b = service.card("poznan", "Repertuar kin w Poznaniu", "Kinowo", "kinowo.net",fiveFilms)
  ...
  it should "degrade to a poster-less card (still 1200×630) when no poster decodes" in {
    ...
    dimensions(new CityOgCardService(fetch).card("poznan", "Repertuar kin w Poznaniu", "Kinowo", "kinowo.net",fiveFilms)) shouldBe (1200, 630)
  }
  ...
  it should "not cache a poster-less card, so the next share retries the fetch" in {
    ...
    val service = new CityOgCardService(fetch)
    val first  = service.card("poznan", "Repertuar kin w Poznaniu", "Kinowo", "kinowo.net",fiveFilms)
    val second = service.card("poznan", "Repertuar kin w Poznaniu", "Kinowo", "kinowo.net",fiveFilms)
  ...
  it should "not touch the network when the city has no films" in {
    val fetch = new CountingFetch(jpeg)
    new CityOgCardService(fetch).card("poznan", "Repertuar kin w Poznaniu", "Kinowo", "kinowo.net",Seq.empty)
  ...
  it should "walk past a blocked primary poster to a reachable cinema fallback" in {
    ...
    val films = Seq.fill(5)(CityCardFilm("Incepcja", Seq("2010"),
      ...
      "Sobota 20 czerwca", Seq("Multikino" -> Seq("18:30"))))
    posterRed(new CityOgCardService(fetch).card("poznan", "Repertuar kin w Poznaniu", "Kinowo", "kinowo.net",films)) should be > 150
  }
}
