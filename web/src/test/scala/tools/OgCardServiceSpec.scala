...
class OgCardServiceSpec extends AnyFlatSpec with Matchers {
  ...
  // Serves the payload only for URLs whose host the Fly datacenter IP can reach;
  // every other URL (e.g. a Cloudflare-blocked www.multikino.pl origin) returns
  // None — models the prod block where the primary poster's host 403s our IP but
  // a fallback origin doesn't.
  private class HostGatedFetch(payload: Array[Byte], reachableHostFragment: String) extends PosterFetch {
    val calls = new AtomicInteger(0)
    override def bytes(url: String): Option[Array[Byte]] = {
      calls.incrementAndGet()
      if (url.contains(reachableHostFragment)) Some(payload) else None
    }
  }
  ...
  it should "fall back to the next candidate poster URL when the primary host blocks our IP (Multikino)" in {
    // Primary is a Multikino origin Cloudflare 403s from the Fly datacenter IP
    // (and weserv SkipHosts it, so the proxy can't rescue it either); the fallback
    // chain carries a reachable cinema-city origin. The card must walk past the
    // dead primary to the working fallback rather than render text-only.
    val fetch = new HostGatedFetch(jpeg, reachableHostFragment = "cinema-city.pl")
  ...
  it should "stop at the first candidate that loads, leaving later fallbacks unfetched" in {
    val fetch = new CountingFetch(jpeg)
