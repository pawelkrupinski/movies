...
object FixtureServerMain {
  ...
  def main(args: Array[String]): Unit = {
    ...
    def indexPageFor(c: City): String = {
      ...
      views.html.repertoire(schedulesFor(c), c.cinemaDisplayNames, c.cinemaPillMap, devMode = false, minifier = tools.Minify, pinnedToday = wiring.pinnedToday,
        oauthProviders = oauthConfigured, renderedAt = now).body
    }
    def browsePageFor(c: City): String = {
      implicit val ci: City = c
      views.html.browse(schedulesFor(c), "Filmy", minifier = tools.Minify, pinnedToday = wiring.pinnedToday, oauthProviders = oauthConfigured).body
    }
    ...
    def manyCinemaFilmPageFor(c: City): String = {
      ...
      schedulesFor(c).headOption match {
        case Some(base) => views.html.film(ManyCinemaFilm(base),
          s"http://test.local/${c.slug}/movie-many", ogDescription = "", minifier = tools.Minify, pinnedToday = wiring.pinnedToday).body
        case None       => "<html><body>Film not found</body></html>"
      }
    }
    ...
    def manyShowtimesPageFor(c: City): String = {
      ...
      views.html.repertoire(bumped, c.cinemaDisplayNames, c.cinemaPillMap, devMode = false, minifier = tools.Minify, pinnedToday = wiring.pinnedToday,
