...
trait UsersWiring { self: Wiring =>
  ...
  lazy val userStateRepository: UserStateRepository = UsersWiring.podUserStateRepository(new MongoUserStateRepository(usersConnection.database, fallbackToOwnInit = false, writeOutcomes = userStateWriteMetrics))
