...
final class RepertoireCacheLastModifiedTests: XCTestCase {
    ...
    func testCachedBodyIsNilForADifferentDeploymentOfTheSameCity() {
        cache.save([], deployment: poland, city: "berlin", lastModified: nil)
        XCTAssertNil(cache.load(deployment: germany, city: "berlin"))
    }

    func testCachedBodyRoundTripsForTheDeploymentThatProducedIt() {
        cache.save([], deployment: germany, city: "berlin", lastModified: nil)
        XCTAssertEqual(cache.load(deployment: germany, city: "berlin")?.count, 0)
    }
    ...
    func testNotModifiedHandsBackTheCachedBodyWhenTheCallerHasNothing() {
        ...
        XCTAssertNotNil(cache.bodyForNotModified(
    ...
    func testNotModifiedHandsBackNothingForADifferentDeployment() {
        cache.save([], deployment: poland, city: "berlin", lastModified: "x")
        XCTAssertNil(cache.bodyForNotModified(
            callerIsEmpty: true, deployment: germany, city: "berlin"))
    }
