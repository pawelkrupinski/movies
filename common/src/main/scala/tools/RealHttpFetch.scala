...
class RealHttpFetch(proxy: Option[RealHttpFetch.ProxyConfig] = None) extends HttpFetch with Logging {
  ...
  // per-host (see HostPolicies): the default 30s, but a stall-prone
  ...
  private val clientsByConnectTimeout: Map[Duration, HttpClient] =
    (HostPolicies.DefaultConnectTimeout +: HostPolicies.all.map(_.connectTimeout))
      .distinct
  ...
  private[tools] def clientFor(url: String): HttpClient =
    clientsByConnectTimeout(HostPolicies.connectTimeoutFor(url))
  ...
  private[tools] def postRequest(url: String, body: String, contentType: String): HttpRequest = {
    ...
    HostPolicies.headersFor(url).foreach { case (k, v) => builder.setHeader(k, v) }
    builder.build()
  }
