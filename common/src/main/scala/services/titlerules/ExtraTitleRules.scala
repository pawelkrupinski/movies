...
object ExtraTitleRules {
  ...
  val searchStrips: Seq[TitleRule] = Seq(
    ...
    searchStrip("xtra-pipe-cycle-banner",           """(?iu)\s*\|\s*(?:Cykl|Przegląd|Festiwal|DKF|FKS|Retrospektywa|Klasyk[ai]|Klasyczne|Klub\s+Filmowy|Poniedziałki\s+z|Wtorki\s+z|Młode\s+Horyzonty|Kinoteka|Filmowy\s+Wałbrzych|Pokaz\s+przedpremierowy|Przedpremiera|Kino\s+przy\s+herbatce|Kino\s+dyskomfortu|Tylko\s+sztuka|Eko-Kino|10/10)\b.*$""", "'<film> | <cycle/festival banner>' generic PIPE-suffix strip, keyword-guarded so it only fires when the segment right after the pipe is a recognised cycle word — never amputates a 'Banner | Film' shape where the film FOLLOWS the pipe (KINO SENIORA | Ojczyzna, LATO w LUNIE | Drzewo magii, FEDERICO FELLINI: ciao a tutti! | GŁOS Z KSIĘŻYCA stay intact). The keyword list is the whole rule: a 2026-09-05 audit found 56 PL films known ONLY by a pipe-suffixed strand, 32 of them (57%, against a ~26% corpus baseline) with no TMDB id because the strand went into the search query — 'Młode Horyzonty' alone accounted for 52. Widening the guard is how this rule grows; loosening it is how it eats a film. ONE MEASURED SIDE EFFECT, traced end to end 2026-09-05: a decorated row stops INHERITING the base film\'s release year. The Kinoteka listing of \"90. urodziny Pavarottiego\" has its own detail page, and that page states no year at all (only a 95-minute runtime); the 2026 it used to carry came from the BARE row, whose detail does state it. Stripping the strand keeps the two apart, so `MovieCache.venueSlotYear` finds no year on the venue\'s slot and the row is never promoted from a yearless key onto 2026. That is the rule working — a decorated edition is not the base film — and the row keeps the year its own source actually gives, which is none. It costs that card its year pill, against 52 films whose TMDB query this fixes and a fixture row (\"Kolor purpury | Kinoteka jest Kobietą\") that goes from a 2026 key with no synopsis to Spielberg\'s 1985 film. Worth knowing because it means a search-tier keyword CAN move a stored key year, by way of what the row is allowed to be merged with"),
