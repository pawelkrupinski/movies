...
object ExtraTitleRules {
  ...
  val searchStrips: Seq[TitleRule] = Seq(
    ...
    searchStrip("xtra-pipe-cycle-banner",           """(?iu)\s*\|\s*(?:Cykl|Przegląd|Festiwal|DKF|FKS|Retrospektywa|Klasyk[ai]|Klasyczne|Klub\s+Filmowy|Poniedziałki\s+z|Wtorki\s+z|Młode\s+Horyzonty|Kinoteka|Filmowy\s+Wałbrzych|Pokaz\s+przedpremierowy|Przedpremiera|Kino\s+przy\s+herbatce|Kino\s+dyskomfortu|Tylko\s+sztuka|Eko-Kino|10/10)\b.*$""", "'<film> | <cycle/festival banner>' generic PIPE-suffix strip, keyword-guarded so it only fires when the segment right after the pipe is a recognised cycle word — never amputates a 'Banner | Film' shape where the film FOLLOWS the pipe (KINO SENIORA | Ojczyzna, LATO w LUNIE | Drzewo magii, FEDERICO FELLINI: ciao a tutti! | GŁOS Z KSIĘŻYCA stay intact). The keyword list is the whole rule: a 2026-09-05 audit found 56 PL films known ONLY by a pipe-suffixed strand, 32 of them (57%, against a ~26% corpus baseline) with no TMDB id because the strand went into the search query — 'Młode Horyzonty' alone accounted for 52. Widening the guard is how this rule grows; loosening it is how it eats a film. ONE MEASURED SIDE EFFECT, traced 2026-09-05: a decorated row whose strand is stripped can stop acquiring the release year its own DETAIL page supplies — the Kinoteka listing of \"90. urodziny Pavarottiego\" keeps its venue slot yearless, so `MovieCache.venueSlotYear` returns None and the row is never promoted from a yearless key onto 2026. The film is unresolved either way and only its year pill is lost, against 52 films whose TMDB query this fixes and a fixture row (\"Kolor purpury | Kinoteka jest Kobietą\") that goes from a 2026 key with no synopsis to Spielberg\'s 1985 film; but a search-tier rule reaching a stored year at all is the surprising part, and worth knowing before adding a keyword"),
