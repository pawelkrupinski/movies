 ...
 *     don't match a real film), 24h TTL — the miss marker expires after a day;
 *     the re-resolve itself now fires only in the week before a film's first
 *     screening (the phase-spread `PremiereResolveReaper`, which clears each due
 *     row's marker via `clearNegative`). The operator bulk retry can
