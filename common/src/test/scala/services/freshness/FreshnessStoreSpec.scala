...
class FreshnessStoreSpec extends AnyFlatSpec with Matchers {
  ...
  it should "be true within the kind's TTL and false past it" in {
    val store = new InMemoryFreshnessStore
    store.markFresh("scrape|kino-x", CinemaScrape, t0) // 60min default TTL
    store.isFresh("scrape|kino-x", CinemaScrape, t0.plusSeconds(59 * 60)) shouldBe true
    store.isFresh("scrape|kino-x", CinemaScrape, t0.plusSeconds(61 * 60)) shouldBe false
  }

  it should "use the per-kind TTL — a 4h rating stays fresh long after a 60min scrape would be stale" in {
    val store = new InMemoryFreshnessStore
  ...
  "Freshness.ttlFor" should "give 4h for every rating source and a permanent (None) TMDB resolve" in {
    ...
    Freshness.ttlFor(CinemaScrape)  shouldBe Some(60.minutes) // default; tunable via KINOWO_SCRAPE_FRESHNESS_MINUTES
  ...
  it should "let KINOWO_SCRAPE_FRESHNESS_MINUTES override the scrape TTL" in {
    ...
    try {
      ...
      Freshness.ttlFor(CinemaScrape) shouldBe Some(60.minutes)
