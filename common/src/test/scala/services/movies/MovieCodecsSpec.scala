...
class MovieCodecsSpec extends AnyFlatSpec with Matchers {
  ...
  "MovieCodecs" should "round-trip every top-level MovieRecord field" in {
    ...
    val back = StoredMovieDto.toDomain(decoded, titleNormalizer)
  ...
  it should "decode a pre-stamp row's Tmdb slot as an unstamped language" in {
    ...
    StoredMovieDto.toDomain(roundTrip(dto), titleNormalizer).record.data(Tmdb).language shouldBe None
  }
  ...
  /* ── Interning on the DECODE path ─────────────────────────────────────────
   * This is where the worker's resident corpus is actually BORN: `rehydrate`
   * reads every row back through this codec at boot, so a value repeated across
   * N cinema slots arrives as N freshly-allocated strings unless the decoder
   * interns it. `MovieCache.buildCinemaSlot` interns the same fields, but that
   * only ever runs on a re-SCRAPE — and the write-through guard skips a slot
   * whose content is unchanged, so a stable slot could keep its un-interned
   * decoded strings indefinitely.
   *
   * Measured live 2026-08-30: a worker-uk restarted onto a rehydrated corpus held
   * 75.4 MB of `[B` where the 12h-old process it replaced held 57.0 MB — the older
   * process had slowly re-interned itself through re-scrapes. Duplication in the
   * UK corpus, per field: cast 148,724 elements -> 7,465 distinct, filmUrl
   * 32,552 -> 7,435, genres 43,913 -> 54, synopsis 12,650 -> 1,693, posterUrl
   * 27,399 -> 2,115, title 32,552 -> 1,683.
   *
   * `Showtime.bookingUrl` decodes on its own path and stays OUT of the pool: it is
   * per-screening and only 1.6x duplicated, so pooling it would evict this whole
   * low-cardinality vocabulary for almost no saving. */
  private def twoCinemaSlotsAfterRoundTrip(): (SourceData, SourceData) = {
    // `new String` per slot so the values are byte-equal but distinct instances,
    // exactly as two separately-decoded BSON documents produce them. Without this
    // the assertions would pass on javac's constant pool rather than on interning.
    def slot() = SourceData(
      title     = Some(new String("Spider-Man: Brand New Day")),
      synopsis  = Some(new String("A friendly neighbourhood blurb the distributor ships to every cinema.")),
      posterUrl = Some(new String("https://d32qys9a6wm9no.cloudfront.net/images/movies/poster/zjZ3Uhm")),
      cast      = Seq(new String("Tom Holland"), new String("Zendaya")),
      genres    = Seq(new String("Action"))
    )
    val record = MovieRecord(data = Map[Source, SourceData](
      CinemaShowing(Multikino, "spidermanbrandnewday") -> slot(),
      CinemaShowing(Helios,    "spidermanbrandnewday") -> slot()
    ))
    val id      = s"${titleNormalizer.sanitize("Spider-Man: Brand New Day")}|2026"
    val decoded = roundTrip(StoredMovieDto.fromDomain(id, record, Instant.now()))
    val slots   = StoredMovieDto.toDomain(decoded, titleNormalizer).record.data.values.toSeq
    withClue("expected both cinema slots to survive the round trip") { slots should have size 2 }
    (slots.head, slots(1))
  }

  it should "share one title instance across decoded cinema slots" in {
    val (a, b) = twoCinemaSlotsAfterRoundTrip()
    a.title shouldBe Some("Spider-Man: Brand New Day")
    (a.title.get eq b.title.get) shouldBe true
  }

  it should "share one synopsis instance across decoded cinema slots" in {
    val (a, b) = twoCinemaSlotsAfterRoundTrip()
    (a.synopsis.get eq b.synopsis.get) shouldBe true
  }

  it should "share one posterUrl instance across decoded cinema slots" in {
    val (a, b) = twoCinemaSlotsAfterRoundTrip()
    (a.posterUrl.get eq b.posterUrl.get) shouldBe true
  }

  it should "share cast and genre element instances across decoded cinema slots" in {
    val (a, b) = twoCinemaSlotsAfterRoundTrip()
    (a.cast.head   eq b.cast.head)   shouldBe true
    (a.cast(1)     eq b.cast(1))     shouldBe true
    (a.genres.head eq b.genres.head) shouldBe true
  }
}
