  ...
  config = lib.mkIf cfg.enable {
    ...
    services.k3s = {
      ...
      extraFlags = [
        ...
        # NO `--kube-apiserver-arg=oidc-*` HERE ANY MORE. An `oidc` option stood on this role until
        # 2026-09-07, trusting Google as an issuer so Headlamp could present a person's token and
        # have them be a Kubernetes subject in their own right. Sign-in for the fleet moved to
        # oauth2-proxy at the edge, which admits people to a UI rather than to Kubernetes, so the
        # option lost its only call site and the widening no longer bought anything.
        #
        # ⚠️ IF IT COMES BACK, THE USERNAME PREFIX COMES BACK WITH IT. Kubernetes requires a prefix
        # whenever `oidc-username-claim` is not `sub`, and the reason is impersonation: without one,
        # an OIDC token whose email happens to equal a client-certificate CN, or a ServiceAccount's
        # name, would authenticate AS that subject. `oidc:` kept every identity the issuer could
        # mint inside a namespace of its own. Restoring the trust without it would silently widen
        # far more than the diff reads as.
