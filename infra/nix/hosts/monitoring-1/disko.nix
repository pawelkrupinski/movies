{ ... }:
{
  ...
  #
  # The device above is the ROOT disk. Prometheus's TSDB and Grafana's database live on an attached Hetzner
  # Volume, which is a different device and is deliberately NOT declared here -- disko owns disks it
  # may repartition, and the volume is the one device on this machine that must survive a
  # re-conversion untouched.
  ...
  disko.devices.disk.main = {
    device = "/dev/disk/by-id/scsi-0QEMU_QEMU_HARDDISK_126303518";
    type = "disk";
    content = {
      type = "gpt";
      partitions = {
