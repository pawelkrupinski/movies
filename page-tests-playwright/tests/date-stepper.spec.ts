...
// The navbar's ← / → buttons call `stepDate(±1)`, which routes through the
...
// click.
//
// The arrows are a desktop/tablet/landscape affordance — they're hidden on
// portrait phones (where the day pills + search fill the row and swipe handles
// day changes), so this spec skips when the arrows aren't rendered.
...
test.describe('date stepper buttons', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    test.skip(
      !(await page.locator('.date-nav-btn').first().isVisible()),
      'step arrows are hidden on portrait phones',
    );
