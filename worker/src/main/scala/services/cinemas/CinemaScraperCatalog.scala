...
class CinemaScraperCatalog(
  ...
  private val koszalinScrapers     = Seq(helios(HeliosNuxt.Koszalin), new MultikinoClient(mkFetch, "0015", MultikinoKoszalin), new MsiClient(http, "https://bilety.ck105.koszalin.pl", KinoKryterium, today))
  ...
  private val filmwebExtra: Map[String, Seq[CinemaScraper]] = Map(
    ...
    "opole" -> Seq(helios(HeliosNuxt.KedzierzynKozle), new Bilety24OrganizerClient(http, "https://www.bilety24.pl/kino/organizator/kino-bajka-w-kluczborku-1467", KinoBajkaKluczbork), new MsiClient(http, "https://bilety.mok.com.pl", KinoChemik, today, titlePrefix = Some("Chemik")), new FilmwebShowtimesClient(http, 2343, KinoDiana, today = today), new Bilety24OrganizerClient(http, "https://www.bilety24.pl/kino/organizator/krapkowicki-dom-kultury-1244", KinoKrapkowice), new FilmwebShowtimesClient(http, 431, KinoStudio, today = today), new MsiClient(http, "https://bilety.mok.com.pl", KinoTwierdza, today, titlePrefix = Some("TWIERDZA"))),
