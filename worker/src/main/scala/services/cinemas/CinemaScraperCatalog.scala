...
class CinemaScraperCatalog(
  ...
  private val filmwebExtra: Map[String, Seq[CinemaScraper]] = Map(
    ...
    "legnica" -> Seq(new FilmwebShowtimesClient(http, 1420, HeliosLubin, today = today), new FilmwebShowtimesClient(http, 1718, KinoAurum, today = today), new CyfroweKinoClient(http, KinoCyfroweKino), new FilmwebShowtimesClient(http, 373, KinoForumBoleslawiec, today = today), new FilmwebShowtimesClient(http, 288, KinoMuzaLubin, today = today), new FilmwebShowtimesClient(http, 1139, KinoPCA, today = today)),
