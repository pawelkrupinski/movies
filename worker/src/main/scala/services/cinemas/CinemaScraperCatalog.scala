...
class CinemaScraperCatalog(
  ...
  private val opoleScrapers        = Seq(helios(HeliosNuxt.OpoleKarolinka), helios(HeliosNuxt.OpoleSolaris), new EkobiletClient(http, "OPOLSKIELAMY", KinoMeduza))
  ...
  private val slupskScrapers       = Seq(new MultikinoClient(mkFetch, "0030", MultikinoSlupsk), new EkobiletClient(http, "kinorejs", KinoRejs))
  ...
  private val filmwebExtra: Map[String, Seq[CinemaScraper]] = Map(
    ...
    "katowice" -> Seq(cinemaCity("1062", CinemaCity), new FilmwebShowtimesClient(http, 352, KinoPatria, today = today)),
    ...
    "opole" -> Seq(helios(HeliosNuxt.KedzierzynKozle), new Bilety24OrganizerClient(http, "https://www.bilety24.pl/kino/organizator/kino-bajka-w-kluczborku-1467", KinoBajkaKluczbork), new FilmwebShowtimesClient(http, 619, KinoChemik, today = today), new FilmwebShowtimesClient(http, 2343, KinoDiana, today = today), new Bilety24OrganizerClient(http, "https://www.bilety24.pl/kino/organizator/krapkowicki-dom-kultury-1244", KinoKrapkowice), new FilmwebShowtimesClient(http, 431, KinoStudio, today = today), new FilmwebShowtimesClient(http, 1672, KinoTwierdza, today = today)),
    "rybnik" -> Seq(helios(HeliosNuxt.Zory), new Bilety24OrganizerClient(http, "https://www.bilety24.pl/kino/organizator/kino-baltyk-1499", KinoBaltyk), new EkobiletClient(http, "kino-centrum-jastrzebiezdrj", KinoCentrum), new SystemBiletowyClient(http, "https://bilety.mok.zory.pl", KinoNaStarowce), new BiletynaClient(bnFetch, "https://biletyna.pl/Wodzislaw-Slaski/Wodzislawskie-Centrum-Kultury", KinoPegaz), new TeatrZiemiRybnickiejClient(http)),
    ...
    "kalisz" -> Seq(helios(HeliosNuxt.OstrowWielkopolski), new FilmwebShowtimesClient(http, 1513, KinoCentrum3D, today = today), new Bilety24OrganizerClient(http, "https://www.bilety24.pl/kino/organizator/kino-echo-w-jarocinie-1159", KinoEcho), new Bilety24OrganizerClient(http, "https://www.bilety24.pl/kino/organizator/kino-piast-w-ostrzeszowie-601", KinoPiastOstrzeszow), new Bilety24OrganizerClient(http, "https://www.bilety24.pl/kino/organizator/krotoszynski-osrodek-kultury-1668", KinoPrzedwiosnieKrotoszyn)),
    ...
    "tychy" -> Seq(new SystemBiletowyClient(http, "https://ock.systembiletowy.pl", KinoNaszeKino), new FilmwebShowtimesClient(http, 1528, KinoPlanetCinema, today = today)),
    "walbrzych" -> Seq(new MsiClient(http, "https://bilety.nowaruda.pl", KinoMOKNowaRuda, today), new EkobiletClient(http, "mokis-bielawa", KinoMOKiS), new KinoSlezaClient(http, KinoSleza), new BiletynaClient(bnFetch, "https://biletyna.pl/Dzierzoniow/Kinoteatr-Zbyszek", KinoZbyszek), new MultikinoClient(mkFetch, "0041", MultikinoKlodzko), new MultikinoClient(mkFetch, "0043", MultikinoSwidnica)),
    ...
    "nowy-sacz" -> Seq(new EkobiletClient(http, "kino-jaworzyna", KinoJaworzyna), new Bilety24OrganizerClient(http, "https://www.bilety24.pl/kino/organizator/limanowski-dom-kultury-1368", KinoKlaps)),
