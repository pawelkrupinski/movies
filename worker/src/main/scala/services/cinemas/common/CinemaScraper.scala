...
trait CinemaScraper {
  ...
  /** One scrape as `CinemaScrapeRunner` records it: [[fetch]]'s listing, and whether a
   *  FALLBACK served it rather than this venue's own source. Always its own for every
