...
class WorkerWiring extends play.api.Logging {
  ...
  // Re-tries unresolved-TMDB rows only in their PREMIERE WINDOW (the ~week before
  // first screening), phase-spread across a 24h period — the queue-era replacement
  // for MovieService's old daily, all-at-once `retryUnresolvedTmdb` scheduler (it
  // re-dispatched the whole unresolved backlog 10s after boot, the boot ResolveTmdb
  // burst that pinned the shared-CPU credit) AND a retargeting away from re-trying
  // the event/opera long tail forever. `retryResolve` clears each due row's negative
  // + dispatches its ResolveTmdb. Cap bounds a clock-jump/cold burst the same way
  // the rating reaper does — the leftover stays due and re-tries next period.
