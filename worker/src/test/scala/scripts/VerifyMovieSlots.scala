...
object VerifyMovieSlots {
  ...
  case class Report(films: Int, withSlotRows: Int, withEmbedded: Int, withNeither: Seq[String]) {
  ...
  def run(embeddedView: MovieRepository, slots: SlotsRepository): Report = {
    val stored = slots.findAll()
    var films = 0
    var withSlotRows = 0
    var withEmbedded = 0
    val neither = Seq.newBuilder[String]
    embeddedView.foreachRecord { row =>
      films += 1
    ...
    Report(films, withSlotRows, withEmbedded, neither.result())
  }
