...
object CountryRatingRefresh {
  def main(args: Array[String]): Unit = {
    ...
    val conn    = MongoConnection.forCountry(country, services.MongoAddress.fromEnv(tools.Env.fromProcess()).copy(database = Some(country.mongoDb)), required = true, env = tools.Env.fromProcess())
