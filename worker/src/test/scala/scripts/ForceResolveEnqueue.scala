 ...
 *
 * `ResolveMode.Force` is the one sanctioned bypass of `MovieService.needsTmdbResolution`,
  ...
  def all(queue: TaskQueue, rows: Seq[StoredMovieRecord]): Enqueued =
    rows.foldLeft(Enqueued(0, 0)) { (counts, r) =>
      queue.enqueue(
        ...
        EnrichTaskKeys.resolveTmdbPayload(r.title, r.year, mode = ResolveMode.Force)) match {
