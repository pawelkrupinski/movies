...
object RecordGapCinemas {
  def main(args: Array[String]): Unit = {
    ...
    report("Kino Kuźnica (Suchedniów)")(new SystemBiletowyClient(record("visualsoft"), VisualSoftPortal("https://shd.systembiletowy.pl"), KinoKuznica, titles = titleNormalizer).fetch().size)
    // ── Lublin ──
    ...
    report("Farys (Biecz)")(new SystemBiletowyClient(record("visualsoft"), VisualSoftPortal("https://kfb.systembiletowy.pl"), KinoFarys, titles = titleNormalizer).fetch().size)
