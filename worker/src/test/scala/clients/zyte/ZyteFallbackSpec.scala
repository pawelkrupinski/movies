...
class ZyteFallbackSpec extends AnyFlatSpec with Matchers {
  ...
  "fetchFor without a Zyte key" should "return direct unchanged — no proxy in front" in {
    ZyteFallback.fetchFor(direct, unbuilt, new _root_.settings.ProcessConfiguration(_root_.tools.Env.of())) should be theSameInstanceAs direct
  }

  it should "treat a blank key as no key" in {
    ZyteFallback.fetchFor(direct, unbuilt, new _root_.settings.ProcessConfiguration(_root_.tools.Env.of("ZYTE_API_KEY" -> ""))) should be theSameInstanceAs direct
  }

  "fetchFor with a Zyte key" should "front direct with a Zyte fallback chain" in {
    ZyteFallback.fetchFor(direct, new RefusingHttpClient, new _root_.settings.ProcessConfiguration(_root_.tools.Env.of("ZYTE_API_KEY" -> "test-key"))) shouldBe a[FallbackHttpFetch]
  }

  it should "call the Zyte API through the client it was handed" in {
    val client = new RefusingHttpClient
    val chain  = ZyteFallback.fetchFor(direct, client, new _root_.settings.ProcessConfiguration(_root_.tools.Env.of("ZYTE_API_KEY" -> "test-key")))
