 ...
 * and carried their cadence in a `fly.worker*.toml` `[env]` block; those apps and
 * their configs are gone, so `infra/kubernetes/worker/overlays/<cc>/patch.yaml`
 * is now the only place any country's sweep rate exists.
 *
...
class WorkerScrapeCadenceConfigSpec extends AnyFlatSpec with Matchers {
  ...
  /** The k3s overlay that actually deploys a country's worker — the pods on
   *  k3s-worker-1 are what run, and this ConfigMap is the only place a country's
   *  cadence exists. */
  ...
  "the DE worker" should "scrape on a cadence its paced Filmstarts sweep can drain within" in {
    ...
    // originally sized for. See the invariant test below.
    cadenceOf(workerOverlay("de")) shouldBe Some("600")
  }
  ...
  it should "pace Filmstarts slowly enough to stop the 429s, yet still sweep inside that cadence" in {
    ...
    // sweep still fits.
    val pace     = RateLimitedHttpFetch.configuredInterval(new _root_.settings.ProcessConfiguration(tools.Env.of()))("https://www.filmstarts.de/kinoprogramm/kino/A0006/")
  ...
  "the PL worker" should "stay on the hourly cadence" in {
    ...
    cadenceOf(workerOverlay("pl")) shouldBe Some("60")
  }
  ...
  "the UK worker" should "scrape on a cadence its paced Flicks sweep can drain within" in {
    ...
    val pace    = RateLimitedHttpFetch.configuredInterval(new _root_.settings.ProcessConfiguration(tools.Env.of()))("https://www.flicks.co.uk/cinema/sessions/x/2026-07-31/")
  ...
  it should "pace Flicks so the fan-out stops tripping its 429 limiter" in {
    ...
    RateLimitedHttpFetch.configuredInterval(new _root_.settings.ProcessConfiguration(tools.Env.of()))("https://www.flicks.co.uk/cinema/sessions/x/2026-07-31/") should not be empty
  }
  ...
  "the US worker" should "scrape on a cadence its paced Flicks sweep can drain within" in {
    ...
    val pace    = RateLimitedHttpFetch.configuredInterval(new _root_.settings.ProcessConfiguration(tools.Env.of()))("https://www.flicks.us/cinema/sessions/x/2026-08-30/")
  ...
  "the Spain worker" should "scrape on a cadence its paced SensaCine sweep can drain within" in {
    ...
    val pace = RateLimitedHttpFetch.configuredInterval(new _root_.settings.ProcessConfiguration(tools.Env.of()))(
  ...
  it should "keep sensacine.com paced independently of its German sibling" in {
    ...
    val es = RateLimitedHttpFetch.configuredInterval(new _root_.settings.ProcessConfiguration(tools.Env.of()))("https://www.sensacine.com/cines/cine/E0291/")
    val de = RateLimitedHttpFetch.configuredInterval(new _root_.settings.ProcessConfiguration(tools.Env.of()))("https://www.filmstarts.de/kinoprogramm/kino/A0263/")
  ...
  "the oldest-cinema-scrape panel" should "state the same scrape windows the worker apps actually deploy" in {
    ...
    //
    // Every clause is read from that country's k3s OVERLAY, which is what deploys it.
    // A country left out of the sentence entirely is the same failure in a milder form —
    // the panel draws its line and states no band for it.
    val pl = hoursOf(workerOverlay("pl"))
    val uk = hoursOf(workerOverlay("uk"))
    val de = hoursOf(workerOverlay("de"))
