...
object ProjectedMongoCorpus {
  /** Run `body` over a fresh corpus database on `target`, named for `suite`, dropped afterwards. */
  def withCorpus[A](target: IntegrationMongoTarget, suite: String)(body: ProjectedMongoCorpus => A): A =
