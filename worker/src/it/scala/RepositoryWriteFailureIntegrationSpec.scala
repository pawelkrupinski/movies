 ...
 */
class RepositoryWriteFailureIntegrationSpec extends AnyFlatSpec with Matchers with BeforeAndAfterAll with tools.IntegrationMongoSuite {
  ...
  "a side collection's bulk delete that Mongo refuses" should "be counted, and report no rows removed" in {
    val isolatedViewDb = tools.IsolatedMongoDatabase.open(mongoTarget, "repository-delete-failure-spec")
