...
class RepositoryWriteFailureIntegrationSpec extends AnyFlatSpec with Matchers with BeforeAndAfterAll {
  ...
  private val isolatedDb = tools.IsolatedMongoDatabase.open(Env.fromProcess().get("MONGODB_URI").get, "repository-write-failure-spec")
  ...
  "a side collection's bulk delete that Mongo refuses" should "be counted, and report no rows removed" in {
    val isolatedViewDb = tools.IsolatedMongoDatabase.open(Env.fromProcess().get("MONGODB_URI").get, "repository-delete-failure-spec")
