...
class ScanStitchedPagingSpec extends AnyFlatSpec with Matchers {
  ...
  it should "page the side-collection reads instead of preloading them whole" in {
    ...
    tools.IsolatedMongoDatabase.withDatabase(Env.fromProcess().get("MONGODB_URI").get, "scan-stitched-paging") { db =>
      ...
      // The delete cascade is asserted too. This spec once cleared up with a raw `deleteMany`
      // on `movies` alone, which cannot reach the side collections, and stranded `screenings`
      // and `movie_slots` rows keyed to films that no longer existed — a film the next scan's
      // UNION invents. The repository's own `delete` cascades (`screenings.deleteFilm` +
      // `slots.deleteFilm`); the database drop after this would hide it if it stopped.
