...
class DatabaseOwnerIntegrationSpec extends AnyFlatSpec with Matchers with tools.IntegrationMongoSuite {


  "a database" should "be claimed by its first country and refuse every other" in
    tools.IntegrationCorpusDatabase.withDatabase(mongoTarget, "database-owner") { db =>
  ...
  it should "let exactly one of two countries claiming it at once win" in
    tools.IntegrationCorpusDatabase.withDatabase(mongoTarget, "database-owner-race") { db =>
      tools.ConcurrentInstances.rounds(20, tools.ConcurrentInstances.baseSeed(configuration)) { round =>
