...
class FilmScheduleEndToEndSpec extends AnyFlatSpec with Matchers {
  ...
  "the full enrichment pipeline" should
    ...
    if (actual != expected) {
      ...
      info(s"SNAPDIAG2 sorted-equal=${expLines.sorted.sameElements(actLines.sorted)} expLines=${expLines.length} actLines=${actLines.length}")
      val diffs = expLines.zipAll(actLines, "<none>", "<none>").zipWithIndex.collect { case ((e, a), i) if e != a => i }
      info(s"SNAPDIAG2 totalDiffLines=${diffs.length} firstDiffIdx=${diffs.headOption.getOrElse(-1)}")
