...
class HeliosClientFullRepertoireSpec extends AnyFlatSpec with Matchers {
  ...
  it should "return correct production countries for REST-enriched movies" in {
    ...
    byTitle("Drzewo magii").movie.countries                                   shouldBe Seq("Wielka Brytania")
    byTitle("Dyyavol Nosyt' Prada 2 - UA ").movie.countries                   shouldBe Seq("USA")
    byTitle("Kurozając i Świątynia Świstaka").movie.countries                 shouldBe Seq("Francja")
    byTitle("Michael").movie.countries                                        shouldBe Seq("Wielka Brytania", "USA")
    byTitle("Mortal Kombat II").movie.countries                               shouldBe Seq("USA")
    byTitle("Mortal Kombat II - UA").movie.countries                          shouldBe Seq("USA")
    byTitle("Mumia: Film Lee Cronina").movie.countries                        shouldBe Seq("Irlandia", "USA")
    byTitle("Nawet myszy idą do nieba").movie.countries                       shouldBe Seq("Republika Czeska", "Francja", "Inne", "Polska")
    byTitle("O psie, który jeździł koleją ").movie.countries                  shouldBe Seq("Polska")
    byTitle("Obsesja").movie.countries                                        shouldBe Seq("USA")
    byTitle("Projekt Hail Mary").movie.countries                              shouldBe Seq("USA")
    byTitle("Pucio").movie.countries                                          shouldBe Seq("Polska")
    byTitle("Sprawiedliwość owiec").movie.countries                           shouldBe Seq("Niemcy", "Wielka Brytania", "Irlandia", "USA")
    byTitle("Super Mario Galaxy Film").movie.countries                        shouldBe Seq("USA", "Japonia")
    byTitle("Za duży na bajki 3").movie.countries                             shouldBe Seq("Polska")
    // Pure-NUXT entries (event variants without a matching REST movie body) have no REST country.
    byTitle("Cirque du Soleil: Kooza").movie.countries                        shouldBe Seq.empty
    byTitle("Mandalorian & Grogu").movie.countries                            shouldBe Seq.empty
  }
