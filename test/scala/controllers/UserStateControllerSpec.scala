...
import services.users.InMemoryUserStateRepo

import java.time.Instant

class UserStateControllerSpec extends AnyFlatSpec with Matchers {

  private def fixture(prefilled: Option[UserState] = None): (UserStateController, InMemoryUserStateRepo) = {
    val repo = new InMemoryUserStateRepo
    prefilled.foreach(repo.upsert)
    (new UserStateController(Helpers.stubControllerComponents(), repo), repo)
  }
  ...
  it should "replace the user's state with the request body" in {
    ...
    val (ctl, repo) = fixture(Some(initial))
    val request = FakeRequest("PUT", "/api/me/state")
