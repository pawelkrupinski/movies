...
| DB           | Role | Written by |
|--------------|------|------------|
| `kinowo`     | The `/debug` corpus mirror — prod's `movies` synced in, read-only locally | `mirror.sh` tailer |
| `kinowo_dev` | The local **web + worker** read/write playground (its own change streams) | a locally-run worker |

Why the `kinowo` mirror exists: `/debug` renders the whole `movies` corpus
(`movieRepo.findAll()` — a full scan, 1200+ docs). Read over the prod `flyctl`
tunnel that takes **8–30s** and intermittently times out to an empty table; from
the LAN mirror it's **~200ms**.

> One source of truth per collection. The tailer **replaces** `kinowo.movies`
> from prod, so don't also write `kinowo` from a local worker — point the worker
> at `kinowo_dev`. That's the whole reason for two databases.
...
3. **Start the mirror + sync** — either in a terminal (`scripts/local-mirror/mirror.sh`)
   ...
   `kinowo.movies` from prod, then tails prod's change stream into it.
...
Mongo via `brew services` if it's stopped, re-seeds if `kinowo` is empty, and
...
## Running the web + worker stack locally (`kinowo_dev`)

Point both apps at the native Mongo, using the **`kinowo_dev`** database so the
local worker's writes never collide with the prod-synced `kinowo` mirror:

```
MONGODB_URI=mongodb://127.0.0.1:28017/?directConnection=true
MONGODB_DB=kinowo_dev
```

`kinowo_dev` is a normal database on the same replica set, so the read-model
...
`kinowo` (via `MONGODB_MOVIES_MIRROR_URI`); the rest of the local site serves
from whatever the local worker projects into `kinowo_dev`.

## How the mirror stays in sync

`mirror.sh` tails prod's `movies` change stream (`tail.js`) and applies every
insert/update/replace/delete to `kinowo`, persisting a resume token locally so a
...
| 28017 | native local Mongo — `/debug` mirror reads + the sync **target**, and the web+worker `kinowo_dev` |
