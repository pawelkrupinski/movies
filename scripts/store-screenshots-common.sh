...
# The listing's "All dates" pill — proof the LISTING is on screen rather than a
# film's detail. Mirrors R.string.all in each values-<lang>/. NOT the search
# placeholder: that lives in an EditText, and node_center skips those on purpose
# (typing a city name makes the field's own text match the query).
listing_marker() { case "$1" in pl) echo "Wszystkie";; uk) echo "All";; de) echo "Alle";; us) echo "All";; es) echo "Todo";; esac; }
