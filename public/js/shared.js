  ...
  // ── Perf diagnostics ─────────────────────────────────────────────────────
  //
  // Beacons one navigation-timing record per page load to `POST /debug/perf`.
  // Designed for diagnosing the "tap-to-paint" gap on iOS Safari, where the
  // browser keeps the prior page on-screen until the new one is render-
  // ready. Two non-obvious bits:
  //
  //   - **`tapToNavStart`**: bridges from the tap on the *previous* page
  //     to `navigationStart` on this one. The previous page stashes
  //     `Date.now()` in sessionStorage when a `.nav-tab` link is clicked;
  //     boot here reads it and computes `performance.timeOrigin -
  //     stashedClickWallClock`. That's the chunk Navigation Timing
  //     misses — Safari spends it frozen on the old page.
  //
  //   - **`pagehide` + `sendBeacon`**: the only event pair iOS Safari
  //     fires reliably on navigation (bf-cache stashing fires pagehide
  //     too). `sendBeacon` is fire-and-forget so it doesn't hold up the
  //     unload, and the browser keeps the POST queued past navigation.
  //
  // Click capture lives at top level so it's installed before any user
  // interaction; the rest is gated on observers being available so old
  // WebKit just logs less.
  document.addEventListener('click', e => {
    const a = e.target.closest('a.nav-tab');
    if (a) sessionStorage.setItem('navClickAt', String(Date.now()));
  }, { capture: true });

  (function() {
    const samples = { url: location.pathname };
    // `tapToNavStart` has to be computed at boot of the *destination*
    // page — that's when `performance.timeOrigin` (the wall-clock time
    // of this navigation starting) is finally larger than the stashed
    // source-page click timestamp. Reading the stash in `pagehide`
    // would land on the *source* page's pagehide, giving us source's
    // timeOrigin (earlier than the click) and a meaningless negative.
    //
    // Staleness guard: iOS Safari's bf-cache restore reuses the entire
    // JS state, so this IIFE doesn't re-run on a restored tab — the
    // stash from a click hours/days ago can sit in sessionStorage and
    // land in the next non-bf-cache navigation as a wildly negative
    // delta. Only honour stashes < 30 s old; anything older is treated
    // as orphan and cleared.
    const stash = sessionStorage.getItem('navClickAt');
    if (stash) {
      const gap = performance.timeOrigin - Number(stash);
      if (gap >= 0 && gap < 30_000) samples.tapToNavStart = Math.round(gap);
      sessionStorage.removeItem('navClickAt');
    }
    let   lcp = 0, longTaskMs = 0, longTasks = 0;
    ...
    function dump() {
      try {
        performance.getEntriesByType('paint').forEach(e => samples[e.name] = Math.round(e.startTime));
        const nav = performance.getEntriesByType('navigation')[0];
        if (nav) {
          samples.ttfb     = Math.round(nav.responseStart  - nav.requestStart);
          samples.transfer = Math.round(nav.responseEnd    - nav.responseStart);
          samples.domparse = Math.round(nav.domInteractive - nav.responseEnd);
          samples.dcl      = Math.round(nav.domContentLoadedEventEnd);
          samples.load     = Math.round(nav.loadEventEnd);
        }
        samples.lcp        = Math.round(lcp);
        samples.longTaskMs = Math.round(longTaskMs);
        samples.longTasks  = longTasks;
        samples.conn       = (navigator.connection && navigator.connection.effectiveType) || '';
        samples.ua         = navigator.userAgent;
        // `tapToNavStart` was already cached at boot (above) — it lives
        // in `samples` and only makes sense to compute against THIS
        // page's `timeOrigin`, not against the pagehide moment.
        if (navigator.sendBeacon)
          navigator.sendBeacon('/debug/perf', JSON.stringify(samples));
      } catch (_) {}
    }
